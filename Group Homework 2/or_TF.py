...
def analyze_classifier(sess, i, w1, b1, XOR_X, XOR_T):
    ...
    xs = np.linspace(0, 1)quantum_OR_solver.py
    ys = np.linspace(0, 1)
    pred_classes = []
    for x in xs:
        for y in ys:
...
hypothesis = tf.nn.softmax(tf.matmul(input_, w1) + b1)
...
with tf.Session() as sess:
    ...
    for i in range(20001):
        ...
        if i % 10000 == 0:
            analyze_classifier(sess, i, w1, b1, XOR_X, XOR_T)